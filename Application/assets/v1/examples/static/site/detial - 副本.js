define(function(require){
    ...
    if($_GET['seckillproductid'] && $_GET['productid'] || ($_GET['status']=='not'||$_GET['status']=='ing'||$_GET['status']=='over')){
        $('.w_add_cart > div:nth-child(1) i').css('right','33%');
    }
        ...
        var proDetial = {
            ...
            getDetial : function(){
                ...
                if(ret.status == 1){
                    ...
                    priceInfo+='<div>\
                                    <p>\
                                        <span class="sale_price">&yen;<span class="f25">'+data.Price.toFixed(2)+'</span></span>\
                                                        ...
                                                        <ul class="choseSku">'
                                                            $.each(data.SkuInfos,function(k,v){
                                                                if(k == 0){
                                                                    ...
                                                                    addCartShow+='<li class="d_goods_sku_item selected" data-src='+v.Src+' data-price='+v.Price.toFixed(2)+' data-marketprice='+v.MarketPrice.toFixed(2)+' data-skuname="'+v.SkuName+'" data-stockcount='+v.StockCount+' data-skuid="'+v.SkuId+'" data-productid="'+v.ProductId+'"><span>'+v.Size+'</span><span class="icon icon-del"></span></li>'
                                                                }else{
                                                                    addCartShow+='<li class="d_goods_sku_item" data-src='+v.Src+' data-price='+v.Price.toFixed(2)+' data-marketprice='+v.MarketPrice.toFixed(2)+' data-skuname="'+v.SkuName+'" data-stockcount='+v.StockCount+' data-skuid="'+v.SkuId+'" data-productid="'+v.ProductId+'"><span>'+v.Size+'</span><span class="icon icon-del"></span></li>'
                                                                }
                                                            })
                                                ...
                                                <section class="d_goods_size pad4" >\
                                                    ...
                                                    <div class="d_goods_sku_list">\
                                                        <ul class="">'
                                                            $.each(data.SkuInfos,function(k,v){
                                                                if(k == 0){
                                                                    addCartShow+='<li class="d_goods_sku_item selected"><span>'+v.Color+'</span><span class="icon icon-del"></span></li>'
                                                                }else{
    ...
    if(count==0){//表示数组1的这个值没有重复的，放到返回列表中
        arr3.push(stra);
            ...
            getEvaList:function(){
                ...
                if(ret.status == 1){
                    var list = '';
                    if(ret.data.EvaluationList.length <= 0){
                        // $('.d_goods_thought').hide();
                    }else{
                        $.each(ret.data.EvaluationList,function(k,v){
                            if(k <=2 ){
