define(function(require){
	...
	$(document).ready(function(){
		...
		function getprice(){
				...
				var  cir = true;
				function  getluckybagprice(data,num){
						var cir = false;  var cbuyCount = 0; var cluckybagprice=0;
						$.each(data,function(k,v){
								if(v.BuyCount<=num){
										if(cbuyCount<v.BuyCount){
											cbuyCount = v.BuyCount;
											cluckybagprice = v.PackagePrice;
										} 
										cir =true;
								}
						})
						BuyCount += cbuyCount;
						luckybagprice+=cluckybagprice;
						if(cir){
							getluckybagprice(data,num-cbuyCount);
						}
				}
				...
				for (k in luckybag){
					var maxprice = 0, number = 0,BuyCount= 0,luckybagprice = 0, luckybagtolalprice = 0;minBuyCount=0;
							...
							if(number>=minBuyCount&&i>0){
								 getluckybagprice(HotBrandLuckyBagConfigs[k],number);
