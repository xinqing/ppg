...
class Common_util_pub
{
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch, CURLOPT_CAINFO, '/data/wwwroot/ppgV3/Application/rootca.pem');
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class CloseOrder_pub extends Wxpay_client_pub
{
	...
	{		
		try 
		{
			if($this->parameters["out_trade_no"] == null ) 
			{
				throw new SDKRuntimeException("关闭订单接口中，缺少out_trade_no参数！"."<br>");
			}
		   	$this->parameters["appid"] = WxPayConf_pub::APPID;//公众账号ID
		   	$this->parameters["mch_id"] = WxPayConf_pub::MCHID;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
...
{	
	...
	var $AppId="wx79462e5c59d03ecc";
    var $AppSecret="5905d1c1a5bf593d7b5ab003d6cd1b3c";

	/**
	* 	订单提交推送
	*/
  function OrderMsg($OpenId,$orderID,$orderMoneySum){	
       $data='{
           "touser":"'.$OpenId.'",
           "template_id":"8UwuQKAst0eUc0ZeK8LRQdFVn8G1-k9YkPkiL5iypKU",
           "url":"http://m.ppgbuy.com//order/buyList?OrderStatus=1000",       
           "data":{
                   "first": {
                       "value":"恭喜你下单成功！",
                       "color":"#173177"
	...
	* 	商品已发出推送
	*/
  	function ProductMsg($openid,$delivername,$ordername){	
       $data='{
           "touser":"'.$openid.'",
           "template_id":"D6PMYVlnNk396gYOyyE6AlsVmNJYtv92d50PBBJtlAs",
           "url":"http://m.ppgbuy.com//order/buyList?OrderStatus=3000",            
           "data":{
                   "first": {
                       "value":"恭喜你！您的商品已发出！",
                       "color":"#173177"
	...
	* 	订单支付成功通知
	*/
  	function PayMsg($openid,$orderMoneySum,$orderProductName){	
       $data='{
           "touser":"'.$openid.'",
           "template_id":"o3l_juy-zr3VzloH-clrXGZQjYcEQ8ESm4hh7szTCwY",
           "url":"http://m.ppgbuy.com//order/buyList?OrderStatus=2000",            
           "data":{
                   "first": {
                       "value":"恭喜你！您的商品已发出！",
                       "color":"#173177"
    ...
    /*	Header("Location: https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=".$str['ticket']);*/
    	return $str;
    }
}
...
{	
	...
	function createXml()
	{
		try
		{
			...
			{
				throw new SDKRuntimeException("缺少企业付款接口必填参数partner_trade_no！"."<br>");
			}elseif($this->parameters["amount"] == null){
				throw new SDKRuntimeException("缺少企业付款接口必填参数amount！"."<br>");
			}elseif ($this->parameters["desc"] == null ) {
				throw new SDKRuntimeException("缺少企业付款接口必填参数desc！"."<br>");
			}elseif ($this->parameters["check_name"] == null ) {
				throw new SDKRuntimeException("缺少企业付款接口必填参数check_name！"."<br>");
			}elseif ($this->parameters["openid"] == NULL){
				throw new SDKRuntimeException("缺少企业付款接口接口必填参数openid！"."<br>");
			}
...
class JsApi_pub extends Common_util_pub
{
	...
	// 生成用户授权的code
	function createAuthorizationUrlForCode($redirectUrl)
	{
